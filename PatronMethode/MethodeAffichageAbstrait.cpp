...
#include <sstream>
#include <algorithm>
#include "MethodeAffichageAbstrait.h"

using namespace std;

void MethodeAffichageAbstrait::tracerLigne(vector<vector<char>>& grille, int x0, int y0, int x1, int y1){
        // Distance verticale
...
vector<Point> MethodeAffichageAbstrait::creerPoints(const std::string& ligne){
    vector<Point> points;
...
void MethodeAffichageAbstrait::tracerLigne(std::vector<std::vector<char>>& grille, int x0, int y0, int x1, int y1){
        // Distance verticale
    int lignes = abs(y1 - y0);
    // Si la ligne est horizontale 
    if (lignes == 0) {
